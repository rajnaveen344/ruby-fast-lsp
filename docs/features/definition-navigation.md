...
Bracket access retains separate method and argument identities. The indexer
anchors `receiver[key]` and `receiver[key] = value` reference and diagnostic
ranges to the opening `[`; Prism's whole `[key]` message span must not become
a method reference over the key. Argument tokens use their ordinary lexical or
method lookup. Clicking either bracket still selects `[]` or `[]=`, including
inherited implementations. Explicit `receiver.[](key)` retains its `[]` name
range. Extension call facts use the same source-range policy.
Lexical local selection precedes method navigation, including at the first
character after `[`. Cursor containment remains end-inclusive for completion
and navigation immediately after method names.
