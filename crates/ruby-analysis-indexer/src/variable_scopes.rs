...
use ruby_analysis_core::{RubyType, TextRange};
...
impl VariableScopes {
    ...
    pub fn define_variable(&mut self, name: &str, location: TextRange) -> Option<usize> {
        ...
        for var in &scope.local_variables {
            if var.name == name {
                ...
                targets.push(RenameTarget {
                    location: var.definition_location,
        ...
        for scope in &self.scopes {
            for var in &scope.local_variables {
                if var.name == name_key {
                    if var
                        .definition_location
                        .contains_offset(file_id, byte_offset)
                    {
                        return Some(scope.id);
                    }
                    for loc in &var.read_locations {
                        if loc.contains_offset(file_id, byte_offset) {
                            return Some(scope.id);
                        }
                    }
                    for loc in &var.write_locations {
    ...
    pub fn scope_at_position(
        ...
        self.find_scope_at(self.root, file_id, byte_offset)
    }
    ...
    fn find_scope_at(
        ...
        for &child in &scope.children {
            if let Some(found) = self.find_scope_at(child, file_id, byte_offset) {
                return Some(found);
            }
        }
        ...
        if scope.range.contains_offset(file_id, byte_offset) {
            Some(scope_id)
        } else {
            None
        }
...
pub struct VariableNode {
    pub name: ustr::Ustr,
    pub definition_location: TextRange,
    pub read_locations: Vec<TextRange>,
    pub write_locations: Vec<TextRange>,
