...
use crate::core::{FullyQualifiedName, LiteralKey, RubyMethod, RubyType, TextRange};
...
impl CallableBodySummary {
    ...
    pub(crate) fn validate(&self) -> Result<(), crate::core::UnknownReason> {
        ...
        {
            return Err(crate::core::UnknownReason::CallableBodyBoundExceeded);
        }
        if self.captures.windows(2).any(|pair| pair[0] >= pair[1]) {
            return Err(crate::core::UnknownReason::UnsupportedCallableBody);
        }
...
fn validate_expression(
    ...
    depth: usize,
    nodes: &mut usize,
) -> Result<(), crate::core::UnknownReason> {
    if depth > MAX_CALLABLE_BODY_TYPE_DEPTH {
        return Err(crate::core::UnknownReason::CallableBodyBoundExceeded);
    }
    *nodes = nodes
        .checked_add(1)
        .ok_or(crate::core::UnknownReason::CallableBodyBoundExceeded)?;
