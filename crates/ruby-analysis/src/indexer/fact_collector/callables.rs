...
impl FactCollector {
    ...
    pub(super) fn invalidate_callable_names(&mut self, names: HashSet<String>) {
        for name in names {
    ...
    pub(super) fn invalidate_escaped_callables_in_call(&mut self, node: &CallNode<'_>) {
        if self.flow.local_callables.is_empty() {
            return;
        }
        let mut escaped = EscapedCallableReadCollector::default();
    ...
    pub(super) fn merge_local_callables(
    ...
    pub(super) fn prepare_higher_order_call_for_node(
        &self,
    ...
    pub(super) fn infer_rbs_higher_order_call_outcome(
        &self,
        call_node: &CallNode<'_>,
        local_types: &HashMap<String, RubyType>,
        prepared: Option<
    ...
    pub(super) fn infer_yielding_block_return_type_for_call(
        &self,
        call_node: &CallNode<'_>,
    ) -> Option<RubyType> {
    ...
    pub(super) fn instantiate_known_proc_with_stack(
        &self,
    ...
    pub(super) fn infer_proc_body(&self, body: Option<Node<'_>>) -> Option<RubyType> {
    ...
    pub(super) fn infer_proc_call_return_type(
        &self,
        call_node: &CallNode<'_>,
        local_types: &HashMap<String, RubyType>,
    ) -> Option<TypeInferenceOutcome> {
    ...
    pub(super) fn constant_callable_body_for_node(
        &self,
        node: &Node<'_>,
    ...
    pub(super) fn infer_block_param_types_for_call(
        &self,
        node: &CallNode<'_>,
    ) -> (
        Vec<RubyType>,
    ...
    pub(super) fn infer_block_param_types_from_yielding_method(
        &self,
        node: &CallNode<'_>,
    ) -> Option<Vec<RubyType>> {
    ...
    pub(super) fn record_current_method_yield_types(&mut self, node: &YieldNode<'_>) {
    ...
    pub(super) fn record_current_method_forwarded_yield_types(&mut self, node: &CallNode<'_>) {
...
pub(super) fn block_required_param_count(node: &CallNode<'_>) -> usize {
...
pub(super) fn block_parameter_names(block: &BlockNode<'_>) -> Vec<String> {
...
pub(super) fn numbered_parameter_names(params: NumberedParametersNode<'_>) -> Vec<String> {
    (1..=usize::from(params.maximum()))
        .map(|index| format!("_{index}"))
        .collect()
}

pub(super) fn merge_position_types(existing: &mut Vec<RubyType>, incoming: Vec<RubyType>) {
...
pub(super) fn call_forwards_anonymous_block(node: &CallNode<'_>) -> bool {
    if node
        .block()
