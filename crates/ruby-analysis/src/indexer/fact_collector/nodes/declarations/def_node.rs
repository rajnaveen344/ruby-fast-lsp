...
impl FactCollector {
    pub(in crate::indexer::fact_collector) fn process_def_node_entry(
        &mut self,
        node: &DefNode,
    ) -> bool {
        ...
        let (return_type, return_type_provenance, return_equation) = if is_constructor {
            ...
            // emits receiver candidates and call outcomes. Inferring only on
            // def exit leaves those consumers with the last visited assignment
            // rather than the values reaching the expression on every path.
            self.collect_inferred_method_return(
                node,
    ...
    pub(in crate::indexer::fact_collector) fn install_local_read_types(
        &mut self,
        reads: Vec<LocalReadType>,
    ) {
        if reads.is_empty() {
            return;
        }
    ...
    pub(in crate::indexer::fact_collector) fn process_def_node_exit(&mut self, _node: &DefNode) {
