...
pub(super) struct ExtensionState {
    pub(super) host: Arc<dyn FactCollectorExtensionHost>,
    pub(super) project: Option<ProjectContext>,
    enclosing_calls: Vec<ResolvedCall>,
    call_frames: Vec<ExtensionCallFrame>,
    pub(super) pending_block: Option<BlockExecutionContext>,
}

impl ExtensionState {
    pub(super) fn new(host: Arc<dyn FactCollectorExtensionHost>) -> Self {
        Self {
            host,
            project: None,
    ...
    pub(super) fn enter_call(&mut self, handled: bool, resolved_call: Option<ResolvedCall>) {
    ...
    pub(super) fn current_call_handled(&self) -> bool {
        self.call_frames
            .last()
            .expect(
