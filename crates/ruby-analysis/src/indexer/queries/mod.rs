pub(in crate::indexer) mod code_lens;
pub(in crate::indexer) mod document_symbols;
pub(in crate::indexer) mod hover;
pub(in crate::indexer) mod receivers;
pub(in crate::indexer) mod rename;
pub(in crate::indexer) mod selection_ranges;
pub(in crate::indexer) mod semantic_tokens;
pub(in crate::indexer) mod syntax;
#[cfg(test)]
mod tests;
...
fn push_constant_path_parts(node: &ruby_prism::Node<'_>, namespace_stack: &mut Vec<RubyConstant>) {
    if let Some(cpn) = node.as_constant_path_node() {
        ...
        syntax::collect_namespaces(&cpn, &mut names);
