...
pub(crate) fn lower_callable_literal_with_outer_locals(
    ...
    if captures.len() > crate::core::callables::callable_body::MAX_CALLABLE_BODY_CAPTURES {
    ...
    if expanded_node_count > crate::core::callables::callable_body::MAX_CALLABLE_BODY_NODES {
        return Err(UnknownReason::CallableBodyBoundExceeded);
    }
    let node_count =
...
impl LoweringState {
    ...
    fn make(
        ...
        self.node_count = self
            ...
            .checked_add(1)
            .ok_or(UnknownReason::CallableBodyBoundExceeded)?;
        if self.node_count > crate::core::callables::callable_body::MAX_CALLABLE_BODY_NODES {
            return Err(UnknownReason::CallableBodyBoundExceeded);
        }
        Ok(expression)
    }
    ...
    fn lower_parameters(
        ...
        if total > crate::core::callables::callable_body::MAX_CALLABLE_BODY_PARAMETERS {
            return Err(UnknownReason::CallableBodyBoundExceeded);
        }
