...
use ruby_fast_lsp::parser::{document::RubyDocument, RubyParser};
...
fn test_parser_parse_valid_ruby() {
    ...
    assert!(
        tree.is_some(),
        "Parser should produce a tree for valid Ruby"
    );
    ...
    assert!(
        !root.has_error(),
        "Valid Ruby code should not have syntax errors"
    );
...
fn test_parser_parse_invalid_ruby() {
    ...
    assert!(
        tree.is_some(),
        "Parser should produce a tree even for invalid Ruby"
    );
    ...
    assert!(
        !is_valid_method,
        "Invalid Ruby code (missing 'end') should not have a valid method structure"
    );
}
