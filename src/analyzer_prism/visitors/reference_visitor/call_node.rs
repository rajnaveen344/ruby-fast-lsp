...
impl ReferenceVisitor {
    ...
    fn check_splats(&self, node: &CallNode) -> Vec<UnresolvedEntry> {
        ...
        for arg in args.arguments().iter() {
            ...
            if let Some(splat) = arg.as_splat_node() {
                if let Some(expr) = splat.expression() {
                    if self.is_definitely_non_array(&expr) {
                        let arg_repr =
                            String::from_utf8_lossy(expr.location().as_slice()).to_string();
                        let loc = self
                            .document
                            .prism_location_to_lsp_location(&splat.location());
                ...
                for elem in kw_hash.elements().iter() {
                    if let Some(assoc_splat) = elem.as_assoc_splat_node() {
                        if let Some(expr) = assoc_splat.value() {
                            if self.is_definitely_non_hash(&expr) {
                                let arg_repr =
                                    String::from_utf8_lossy(expr.location().as_slice())
                                        .to_string();
                                let loc = self
                                    .document
                                    .prism_location_to_lsp_location(&assoc_splat.location());
                                entries.push(UnresolvedEntry::bad_splat(
                                    "**".to_string(),
                                    arg_repr,
                                    "Hash".to_string(),
    ...
    fn is_definitely_non_array(&self, expr: &Node) -> bool {
        ...
        if let Some(local) = expr.as_local_variable_read_node() {
            let var_name = String::from_utf8_lossy(local.name().as_slice()).to_string();
            let pos = self.document.offset_to_position(expr.location().start_offset());
            let scopes = self.document.variable_scopes();
            let sid = scopes
    ...
    fn is_definitely_non_hash(&self, expr: &Node) -> bool {
        if expr.as_nil_node().is_some() {
            return false;
        }
