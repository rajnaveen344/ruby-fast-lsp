...
impl Parameter {
    ...
    pub fn new_block(name: String) -> Self {
        Self {
            name,
            param_type: RubyType::Class(crate::types::fully_qualified_name::FullyQualifiedName::try_from("Proc").unwrap()),
            required: false,
            has_default: false,
            keyword: false,
            splat: false,
...
impl MethodSignature {
    ...
    pub fn required_parameters(&self) -> Vec<&Parameter> {
        self.parameters.iter().filter(|p| p.required && !p.splat && !p.double_splat && !p.block).collect()
    }
    ...
    pub fn merge_with(&self, other: &MethodSignature) -> MethodSignature {
        ...
        let merged_return_type = RubyType::union(vec![self.return_type.clone(), other.return_type.clone()]);
...
impl MethodSignatureContext {
        ...
        match class_name {
            Some(class) => {
                if signature.class_method {
                    self.class_methods.entry(class).or_insert_with(Vec::new).push(signature);
                } else {
                    self.instance_methods.entry(class).or_insert_with(Vec::new).push(signature);
                }
            }
            None => {
    ...
    pub fn find_method(&self, class_name: Option<&str>, method_name: &str, is_class_method: bool) -> Vec<&MethodSignature> {
        match class_name {
            Some(class) => {
                ...
                methods
                    .map(|methods| methods.iter().filter(|sig| sig.name == method_name).collect())
                    .unwrap_or_else(Vec::new)
            }
            None => {
                self.global_methods.iter().filter(|sig| sig.name == method_name).collect()
            }
        }
    }
    
    /// Get all method signatures for a class
    pub fn get_class_methods(&self, class_name: &str) -> (Vec<&MethodSignature>, Vec<&MethodSignature>) {
        let instance_methods = self.instance_methods.get(class_name)
            .map(|methods| methods.iter().collect())
            .unwrap_or_else(Vec::new);
            
        let class_methods = self.class_methods.get(class_name)
            .map(|methods| methods.iter().collect())
            .unwrap_or_else(Vec::new);
...
mod tests {
    ...
    fn test_parameter_creation() {
        ...
        let keyword_param = Parameter::new_keyword("verbose".to_string(), RubyType::boolean(), false);
    ...
    fn test_method_call_compatibility() {
        ...
        assert!(!signature.can_call_with(&[RubyType::string(), RubyType::integer(), RubyType::string()])); // Too many arguments
    ...
    fn test_method_signature_context() {
        ...
        let signature = MethodSignature::new(
            ...
            vec![Parameter::new_required("name".to_string(), RubyType::string())],
            RubyType::nil_class(),
        );
    ...
    fn test_signature_merging() {
        let sig1 = MethodSignature::new_inferred(
            "test".to_string(),
            vec![Parameter::new_required("x".to_string(), RubyType::integer())],
            RubyType::string(),
            0.8,
        );
