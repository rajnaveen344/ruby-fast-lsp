...
/// Substitute type variables in RubyType results from the user index.
///
/// When the user index contains methods from RBS (e.g., Enumerable#first → Elem),
/// the return types have unresolved type variables. This function substitutes them
/// using the receiver's type args (e.g., Array[Integer] → Elem becomes Integer).
pub fn substitute_type_vars_in_results(
    return_types: &[RubyType],
    class_name: &str,
    type_args: &[RubyType],
) -> Vec<RubyType> {
    let loader = RBS_LOADER.read();

    // Build substitution map from the class's type_params
    let substitutions = if let Some(class) = loader.get_class(class_name) {
        build_substitution_map(&class.type_params, type_args)
    } else if let Some(module) = loader.get_module(class_name) {
        build_substitution_map(&module.type_params, type_args)
    } else {
        return return_types.to_vec();
    };
    ...
    return_types
        .iter()
        .map(|ty| substitute_ruby_type(ty, &substitutions))
        .collect()
}

/// Substitute type variable names in an already-converted RubyType.
///
/// Handles cases where RBS type variables were stored as `Class(Constant(["Elem"]))`
/// in the user index and need to be replaced with concrete types.
fn substitute_ruby_type(
    ty: &RubyType,
    substitutions: &std::collections::HashMap<String, RubyType>,
) -> RubyType {
    match ty {
        RubyType::Class(fqn) => {
            // Check if this is a type variable name like "Elem"
        ...
        RubyType::Union(types) => {
            let substituted: Vec<RubyType> = types
                .iter()
                .map(|t| substitute_ruby_type(t, substitutions))
                .collect();
            RubyType::union(substituted)
        }
        RubyType::Array(element_types) => {
            let substituted: Vec<RubyType> = element_types
                .iter()
                .map(|t| substitute_ruby_type(t, substitutions))
                .collect();
            RubyType::Array(substituted)
        }
        RubyType::Hash(key_types, value_types) => {
            let sub_keys: Vec<RubyType> = key_types
                .iter()
                .map(|t| substitute_ruby_type(t, substitutions))
                .collect();
            let sub_values: Vec<RubyType> = value_types
                .iter()
                .map(|t| substitute_ruby_type(t, substitutions))
                .collect();
            RubyType::Hash(sub_keys, sub_values)
        }
        _ => ty.clone(),
    }
}
