...
class RubyVM
  ...
  module YJIT
    ...
    def self._print_stats; end

    # Get a list of the YJIT blocks associated with `rb_iseq`
    def self.blocks_for(p1) end

    def self.comments_for(start_address, end_address) end

    def self.disasm(iseq, tty: $stdout&.tty?) end

    def self.disasm_block(cs, block, highlight) end

    def self.enabled?; end

    def self.graphviz_for(iseq) end

    def self.print_counters(counters, prefix:, prompt:) end

    def self.print_sorted_exit_counts(stats, prefix:, how_many: 20, left_pad: 4) end
    ...
    def self.total_exit_count(stats, prefix: 'exit_') end

    private

    # Get a list of the YJIT blocks associated with `rb_iseq`
    def blocks_for(p1) end
    ...
    class Block
      ...
      def outgoing_ids; end
    end

    class Disasm
      def disasm(p1, p2) end
