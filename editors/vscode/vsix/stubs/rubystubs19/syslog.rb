...
module Syslog
  ...
  # strerror(errno).
  def self.log(priority, format_string, *format_args) end
  ...
  def self.mask=(priority_mask) end
  ...
  def self.open(ident, options, facility) end
  ...
  #
  # Arguments are the same as for open().
  def self.reopen(ident, options, facility) end

  private

  # Generates a mask bit for a priority level. See #mask=
  def LOG_MASK(priority_level) end

  # Generates a mask value for priority levels at or below the level specified.
  # See #mask=
  def LOG_UPTO(priority_level) end
  ...
  # replaced with the error message string that would be returned by
  # strerror(errno).
  def log(priority, format_string, *format_args) end
  ...
  #  Syslog.open("webrick", Syslog::LOG_PID,
  #              Syslog::LOG_DAEMON | Syslog::LOG_LOCAL3)
  def open(ident, options, facility) end
  ...
  #
  # Arguments are the same as for open().
  def reopen(ident, options, facility) end
  alias open! reopen
