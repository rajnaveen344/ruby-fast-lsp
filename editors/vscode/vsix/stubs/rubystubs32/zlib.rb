...
module Zlib
  ...
  def self.adler32(string = _, adler = _) end
  ...
  def self.deflate(string, level = _) end
  ...
  def self.gzip(src, level: nil, strategy: nil) end
  ...
  class Deflate < ZStream
    ...
    def initialize(level = DEFAULT_COMPRESSION, window_bits = MAX_WBITS, mem_level = DEF_MEM_LEVEL, strategy = DEFAULT_STRATEGY) end
    ...
    #
    # See the constants for further description.
    def deflate(string, flush = Zlib::NO_FLUSH) end
    ...
    def flush(flush = Zlib::SYNC_FLUSH) end
  ...
  class GzipWriter < GzipFile
    ...
    # +:external_encoding+, +:internal_encoding+ and +:encoding+ may be set as in
    # IO::new.
    def initialize(io, level = nil, strategy = nil, options = {}) end
  ...
  class Inflate < ZStream
    ...
    #     end
    #   end
    def initialize(window_bits = Zlib::MAX_WBITS) end
    ...
    def inflate(deflate_string, buffer: nil) end
